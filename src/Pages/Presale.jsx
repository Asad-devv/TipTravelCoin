...
const contractAddress = '0x9bC0e798167704a1163479367DF1F307D8934aC3'; // Your token contract address
const USDTAddress = '0x7e3AA075cA183BA96F71c8C180f90B8fD304D278'; // USDT contract address (mainnet)

const wagmiContractConfig = {
  address: '0x7e3AA075cA183BA96F71c8C180f90B8fD304D278', // USDT contract address
  abi: [
    {
      type: 'function',
      name: 'balanceOf',
...
const [showSuccessModal, setShowSuccessModal] = useState(false);
  ...
  const calculateTokens = () => {
    ...
    if (paymentMethod === 'ETH') {
      const tokenRate = 40000; // 1 ETH = 40,000 tokens (adjust if needed)
      return (amt * tokenRate).toLocaleString();
    } else {
  ...
  const approveUSDT = async () => {
    ...
    try {
      ...
      const tokenAmount = ethers.utils.parseUnits(calculateTokens(), 18); // assuming USDT has 18 decimals
  ...
  const buyTokenWithUSDT = async () => {
    ...
    try {
      ...
      const tokenAmount = ethers.utils.parseUnits(calculateTokens(), 9);
